      ...
      lib.nameValuePair "wireguard-${connectionName}-private-key-env" {
        ...
        content = ''
          WIREGUARD_PRIVATE_KEY=${
            config.sops.placeholder.${profile.privateKeySecret}
          }
        '';
      })
    cfg.profiles;
...
networking.networkmanager.ensureProfiles = {
  environmentFiles =
    lib.mapAttrsToList
      (connectionName: _:
        config.sops.templates."wireguard-${connectionName}-private-key-env".path)
      cfg.profiles;
    ...
    lib.mapAttrs
      (connectionName: profile: {
        ...
        "wireguard-peer.${profile.serverPublicKey}" = {
          ...
          allowed-ips =
            lib.concatStringsSep ";" profile.allowedIPs;
        ...
        ipv4 = {
          ...
          addresses =
            lib.concatStringsSep ";" profile.address;
        };
